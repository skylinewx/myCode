 ...
 */
@Component
public class MyDistributedLocalCacheServiceImpl implements MyService {

    private final ReentrantReadWriteLock readWriteLock = new ReentrantReadWriteLock();
    @Autowired
    private MyCache<MyObj> myObjMyCache;
    @Autowired
    private MyDao myDao;
    ...
    public void getById(String id, IAfterGet afterGet){
        ...
        readLock.lock();
        try {
            MyObj myObj = myObjMyCache.get(id);
            if (myObj == null) {
                //缓存未命中，释放读锁
                readLock.unlock();
                ReentrantReadWriteLock.WriteLock writeLock = readWriteLock.writeLock();
    ...
    public void save(MyObj myObj){
        ...
        try {
            ...
            myDao.save(myObj);
            //缓存失效
            myObjMyCache.remove(myObj.getId());
        }finally {
            writeLock.unlock();
        }
    }
